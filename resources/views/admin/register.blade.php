{{-- FILE: resources/views/registration/register.blade.php --}}
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="container">
  ...
  <form id="regForm" novalidate>
    ...
    <div class="field-wrap">
      ...
      {{-- Admin sponsor card shown when "I don't have a Sponsor" is selected --}}
...
<div class="modal-overlay" id="paymentModal">
  <div class="modal-box">
    ...
    <div class="modal-body">
      ...
      {{-- QR — replace asset path with your real QR image --}}
      <div class="payment-qr">
        <img
          src="{{ asset('public/admin/assets/images/HindolMukherjeeQRCode.png') }}"
          alt="Scan to Pay"
        >
...
{{-- ══════════════════════
     ...
     (appears on top of everything after payment submitted)
══════════════════════ --}}
...
$(document).on('input', '#fieldReferral', function () {
    let code = $(this).val();
    $.ajax({
        url: '{{ route("sponsor.check-referral-verification") }}',
        method: 'GET',
        data: {
            _token: '{{ csrf_token() }}',
            referral_code: code
...
async function fetchSponsorResults(q) {
  try {
    const res  = await fetch('{{ route("sponsor.search") }}?q=' + encodeURIComponent(q), {
      headers: {
...
document.getElementById('submitPayBtn').addEventListener('click', async function () {
  ...
  try {
    ...
    const res    = await fetch('{{ route("payment.submit") }}', {
      method : 'POST',
