...
class MemberstpschedulesController extends Controller
{
    ...
    public function index()
    {
        ...
        $schedules = Memberstpschedule::where('member_id', $memberID)
                        ->orderBy('id', 'desc')
                        ->get();
        ...
        return view('member.memberstpschedules', compact('schedules', 'memberInfos'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Determine who is adding — store numeric member_id (int)
        $addedById = null;
        if (session('type') == 'Admin') {
            $addedById = session('admin_id'); // already int
        } elseif (session('type') == 'Member') {
            // session('member_memberID') is like 'SB0783633087' — fetch numeric id
        ...
        return redirect()->route('member.memberstpschedules.index')
