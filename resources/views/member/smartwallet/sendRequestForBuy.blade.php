...
$(document).ready(function () {   
    ...
    $('#sentRfbHistoryTable').DataTable({
        processing: true,
        serverSide: false,
        ajax: "{{ route('member.smartwallet.buySell.rfbListData') }}",
        columns: [
            { data: 'DT_RowIndex' },
            { data: 'rfb_id' },
            { data: 'amount' },
    ...
    $('#sendRequestBtn').on('click', function () {
        ...
        $.ajax({
            url: "{{ route('member.smartwallet.buySell.sendRequestForBuyStore') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                sellers: selected,
    ...
    $(document).on('click', '.transfer-btn', function () {
        ...
        $.ajax({
            url: "{{ route('member.smartwallet.buySell.sellerAcceptDetails', ':id') }}".replace(':id', rfbId),
            type: "GET",
    ...
    $(document).on('click', '#submitTransferMoney', function () {
        ...
        $.ajax({
            url: "{{ route('member.smartwallet.buySell.transferMoneyStore') }}",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
