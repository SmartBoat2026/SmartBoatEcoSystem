...
<main class="main" id="main" role="main">
    ...
    <div class="card shadow-sm">
        <div class="card-body table-responsive">
            ...
            <div id="bulkActionBar"
                ...
                <form id="bulkDeleteForm" method="POST" action="{{ route('managereport.bulkDelete') }}" style="display:inline;">
                    @csrf
                    ...
                    @forelse($reports as $key => $row)
                        <tr>
                            <td class="text-center">
                                <input type="checkbox" class="row-checkbox" value="{{ $row->member_id }}"
                            ...
                            <td>{{ $row->sponser_name_resolved }}</td>

                            <td>
                                @if($row->joining_member_date)
                                    @php
                            ...
                            {{-- ── TRANSACTION PASSWORD (masked, click to reveal) ── --}}
                            <td>
                                <span class="pass-mask" style="cursor:pointer;letter-spacing:2px;"
                                      data-pass="{{ $row->transaction_password }}"
                                      title="Click to reveal">••••••••</span>
                            </td>

                            <td>{{ $row->amount }}</td>
                            <td>{{ $row->smart_wallet_balance }}</td>

                            <td>{{ $row->total_smartpoint }}</td>
                            <td>{{ $row->total_smartquantity }}</td>
                            ...
                            <td>
                                <div class="d-flex gap-1 justify-content-center">
                                    ...
                                    {{-- ACCESS button — opens member dashboard in new tab --}}
                                    <form method="POST"
                                        action="{{ route('managereport.access') }}"
                                        ...
                                        <button type="submit"
                                                ...
                                                onclick="return confirm('Access dashboard as {{ addslashes($row->name) }} ({{ $row->memberID }})?')">
                        ...
                        <div class="modal fade" id="editModal{{ $row->member_id }}" tabindex="-1">
                            <div class="modal-dialog">
                                <form method="POST" action="{{ route('managereport.update', $row->member_id) }}">
                                    @csrf
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title">Edit Member — {{ $row->memberID }}</h5>
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <input type="text" name="memberID" value="{{ $row->memberID }}" class="form-control" required>
                                            </div>
                                            <div class="mb-2">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <div class="input-group">
                                                    <select name="phone_code_edit_{{ $row->member_id }}"
                                                            class="form-select phone-code-select"
                                                            style="max-width:130px;"
                                                            data-target="editPhone_{{ $row->member_id }}">
                                                        <option value="+91"  {{ $editDialCode === '+91'  ? 'selected' : '' }}>🇮🇳 +91 India</option>
                                                        <option value="+880" {{ $editDialCode === '+880' ? 'selected' : '' }}>🇧🇩 +880 BD</option>
                                                        <option value="+1"   {{ $editDialCode === '+1'   ? 'selected' : '' }}>🇺🇸 +1 USA</option>
                                                        <option value="+44"  {{ $editDialCode === '+44'  ? 'selected' : '' }}>🇬🇧 +44 UK</option>
                                                        <option value="+971" {{ $editDialCode === '+971' ? 'selected' : '' }}>🇦🇪 +971 UAE</option>
                                                        <option value="+966" {{ $editDialCode === '+966' ? 'selected' : '' }}>🇸🇦 +966 KSA</option>
                                                        <option value="+60"  {{ $editDialCode === '+60'  ? 'selected' : '' }}>🇲🇾 +60 MY</option>
                                                        <option value="+65"  {{ $editDialCode === '+65'  ? 'selected' : '' }}>🇸🇬 +65 SG</option>
                                                        <option value="+61"  {{ $editDialCode === '+61'  ? 'selected' : '' }}>🇦🇺 +61 AU</option>
                                                        <option value="+49"  {{ $editDialCode === '+49'  ? 'selected' : '' }}>🇩🇪 +49 DE</option>
                                                        <option value="+33"  {{ $editDialCode === '+33'  ? 'selected' : '' }}>🇫🇷 +33 FR</option>
                                                        <option value="+81"  {{ $editDialCode === '+81'  ? 'selected' : '' }}>🇯🇵 +81 JP</option>
                                                        <option value="+86"  {{ $editDialCode === '+86'  ? 'selected' : '' }}>🇨🇳 +86 CN</option>
                                                        <option value="+7"   {{ $editDialCode === '+7'   ? 'selected' : '' }}>🇷🇺 +7 RU</option>
                                                        <option value="+55"  {{ $editDialCode === '+55'  ? 'selected' : '' }}>🇧🇷 +55 BR</option>
                                                        <option value="+92"  {{ $editDialCode === '+92'  ? 'selected' : '' }}>🇵🇰 +92 PK</option>
                                                        <option value="+94"  {{ $editDialCode === '+94'  ? 'selected' : '' }}>🇱🇰 +94 LK</option>
                                                        <option value="+977" {{ $editDialCode === '+977' ? 'selected' : '' }}>🇳🇵 +977 NP</option>
                                                        <option value="+95"  {{ $editDialCode === '+95'  ? 'selected' : '' }}>🇲🇲 +95 MM</option>
                                                        <option value="+66"  {{ $editDialCode === '+66'  ? 'selected' : '' }}>🇹🇭 +66 TH</option>
                                                    </select>
                                                    <input type="tel"
                                                           id="editPhone_{{ $row->member_id }}"
                                                           class="form-control phone-number-input"
                                                           placeholder="Phone number"
                                                           value="{{ $editNumber }}">
                                                    <input type="hidden"
                                                           name="phone"
                                                           id="editPhoneHidden_{{ $row->member_id }}"
                                                           value="{{ $storedPhone }}">
                                                </div>
                                            </div>
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <input type="text" name="transaction_password" value="{{ $row->transaction_password }}" class="form-control">
                                            </div>
                                            <div class="mb-2">
                                                <label class="form-label fw-semibold">Sponsor ID</label>
                                                <input type="text" name="sponser_id" value="{{ $row->sponser_id }}" class="form-control">
                                            </div>
                                            <div class="mb-2">
                                                <label class="form-label fw-semibold">Sponsor Name</label>
                                                <input type="text" name="sponser_name" value="{{ $row->sponser_name_resolved }}" class="form-control">
                                            </div>
                                            <div class="mb-2">
                                                <label class="form-label fw-semibold">Joining Date</label>
                                                <input type="date" name="joining_date" value="{{ $row->joining_date }}" class="form-control" required>
                                            </div>
                                            <div class="mb-2">
                                                <label class="form-label fw-semibold">Smart Point</label>
                                                <input type="number" name="smart_point" value="{{ $row->smart_point }}" class="form-control">
                                            </div>
    ...
    <div class="modal fade" id="addModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <form method="POST" action="{{ route('managereport.store') }}" id="addMemberForm">
                @csrf
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-2">
                            ...
                            <div class="input-group">
                                ...
                                {{-- Hidden field that stores dial-code + number and is submitted --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var memberPass   = @json(session('new_pass'));
        var memberTxn    = @json(session('new_password'));
        var memberAge    = @json(session('new_age'));
        var memberGender = @json(session('new_gender'));
