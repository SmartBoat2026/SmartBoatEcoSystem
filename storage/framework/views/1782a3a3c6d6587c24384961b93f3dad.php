        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="<?php echo e(route('productpurchase.store')); ?>" id="purchaseForm">
                        <?php echo csrf_field(); ?>
...
<div id="adminWalletBlock" style="display:none;" class="mb-3">
    <div style="background:linear-gradient(90deg,#1a3a6b,#0c447c);border-radius:8px;padding:12px 18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div>
                <div style="font-size:10px;color:rgba(255,255,255,0.65);text-transform:uppercase;letter-spacing:.1em;font-weight:700;">Member Smart Wallet Balance</div>
                <div id="adminWalletAmount" style="font-size:20px;font-weight:900;color:#fff;line-height:1.1;">₹0.00</div>
            </div>
        </div>
        <span id="adminWalletStatus" style="font-size:11px;padding:4px 14px;border-radius:20px;font-weight:700;"></span>
    </div>
</div>
...
<?php endif; ?>
                ...
                <div class="modal-footer" style="border-top:0.5px solid #dee2e6;">
                    ...
                    <button type="submit" form="purchaseForm" class="btn btn-primary btn-sm px-4" id="submitBillBtn" disabled>Create Bill</button>
                </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive p-3">
                ...
                <div id="bulkActionBar" style="display:none;background:#fff3cd;border:1px solid #ffc107;border-radius:6px;padding:10px 16px;margin-bottom:12px;align-items:center;gap:12px;">
                    ...
                    <form id="bulkDeleteForm" method="POST" action="<?php echo e(route('productpurchase.bulkDelete')); ?>" style="display:inline;">
                        <?php echo csrf_field(); ?>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="font-size:12px;color:#6c757d;white-space:nowrap;">
                                <div style="font-weight:500;color:#333;"><?php echo e(\Carbon\Carbon::parse($pur->purchase_date)->format('d M Y')); ?></div>
                                <div style="font-size:11px;color:#adb5bd;"><?php echo e(\Carbon\Carbon::parse($pur->purchase_date)->format('h:i A')); ?></div>
                            </td>
                            <td>
                            ...
                            <td><?php echo e(number_format($pur->total_smartquantity, 4)); ?></td>
                            <td style="color:#27500a;font-weight:500;">₹<?php echo e(number_format($pur->total, 2)); ?></td>
                            <td>
                                <span class="badge <?php echo e($pur->status == 1 ? 'bg-success' : 'bg-danger'); ?>" style="font-size:11px;">
...
$(document).ready(function () {
    ...
    // Wallet state
    let currentWalletBalance = null; // null = no member selected / walk-in
    ...
    // ════════════════════════════════════════════════════════════════
    // 3. WALLET BALANCE HELPERS
    // ════════════════════════════════════════════════════════════════
    function updateWalletDisplay(balance) {
        currentWalletBalance = balance;
        $('#adminWalletAmount').text('₹' + parseFloat(balance).toFixed(2));

        if (parseFloat(balance) > 0) {
            $('#adminWalletStatus').html('<i class="bi bi-check-circle me-1"></i>Available')
                .css({'background':'rgba(44,95,46,0.85)', 'color':'#c0e8a0'});
        } else {
            $('#adminWalletStatus').html('<i class="bi bi-x-circle me-1"></i>No Balance')
                .css({'background':'rgba(220,53,69,0.8)', 'color':'#ffd0d0'});
        }
        $('#adminWalletBlock').show();
        checkWalletAgainstTotal();
    }

    function hideWalletDisplay() {
        currentWalletBalance = null;
        $('#adminWalletBlock').hide();
        $('#walletInlineError').remove();
    }

    function checkWalletAgainstTotal() {
        $('#walletInlineError').remove();
        if (currentWalletBalance === null) return; // walk-in, no check

        const grandTotal = parseFloat($('#grandTotalCell').text().replace('₹','').replace(/,/g,'')) || 0;
        if (grandTotal > 0 && grandTotal > currentWalletBalance) {
            if (!$('#walletInlineError').length) {
                $('#adminWalletBlock').after(
                    `<div id="walletInlineError" class="alert alert-danger d-flex align-items-center gap-2 py-2 mb-2" style="font-size:12px;border-radius:6px;">
                        <i class="bi bi-exclamation-triangle-fill"></i>
                        <div><strong>Insufficient Balance!</strong> Wallet: ₹${parseFloat(currentWalletBalance).toFixed(2)} | Bill: ₹${grandTotal.toFixed(2)}</div>
                    </div>`
                );
            }
        }
    ...
    // Block form submit if wallet insufficient
    $('#purchaseForm').on('submit', function (e) {
        if (currentWalletBalance !== null) {
            ...
            if (grandTotal > currentWalletBalance) {
                e.preventDefault();
                checkWalletAgainstTotal();
                $('#adminWalletBlock')[0].scrollIntoView({ behavior: 'smooth', block: 'center' });
                return false;
            }
        }
    });
    ...
    $('#memberSearchInput').on('input', function () {
        ...
        memberTimer = setTimeout(function () {
            $.get("<?php echo e(route('productpurchase.member')); ?>", { member_id: val })
                .done(function (data) {
                    ...
                    if (data.results && data.results.length > 0) {
                        $.each(data.results, function (i, m) {
                            ...
                            item.on('click', function () {
                                ...
                                $.get("<?php echo e(route('productpurchase.memberWallet')); ?>", { member_id: selId })
                                    .done(function (res) {
                                        if (res.found) {
                                    ...
                                    .fail(function () { hideWalletDisplay(); });
                            });
    ...
    $('#memberDetailType').on('change', function () {
        if ($(this).val() === 'no_id') {
            ...
            $('#memberDropdown').hide();
            hideWalletDisplay(); // ← hide wallet for walk-in
        } else {
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/u642243906/domains/smartboatecosystem.com/public_html/Main/resources/views/admin/productpurchase.blade.php ENDPATH**/ ?>